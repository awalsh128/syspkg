...
var packageInfoPattern = regexp.MustCompile(`Setting up ([\w\d.-]+):?([\w\d]+)? \(([\w\d\.-]+)\)`)
var virtualPkgMsgPattern = regexp.MustCompile(`Can't select versions from package '[^']+' as it is purely virtual`)
	...
	for _, line := range lines {
		if len(line) > 0 {
			...
			case "Section":
				pkg.Category = value
			case "N":
				if virtualPkgMsgPattern.Match([]byte(value)) {
					pkg.AdditionalData["is_virtual"] = "yes"
				}
			}
		}
	}
